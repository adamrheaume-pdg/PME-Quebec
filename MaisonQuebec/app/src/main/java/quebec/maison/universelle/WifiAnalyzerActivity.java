...
public class WifiAnalyzerActivity extends Activity {
    ...
    private final Runnable tick=new Runnable(){public void run(){ if(page==3){refresh(); h.postDelayed(this,4000);} }};
    ...
    private void buildUi(){
        LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(Color.rgb(23,23,23));root.setPadding(12,12,12,0);
        ...
        nav=new LinearLayout(this);nav.setGravity(Gravity.CENTER);String[] n={"▰\nAccès","◎\nCanaux","▥\nGraphe","⌁\nTemps"};for(int i=0;i<4;i++){final int p=i;Button b1=btn(n[i]);b1.setOnClickListener(v->{page=p;h.removeCallbacks(tick);refresh();if(page==3)h.postDelayed(tick,4000);});nav.addView(b1,new LinearLayout.LayoutParams(0,86,1));}root.addView(nav);setContentView(root);
    }
    ...
    private void refresh(){body.removeAllViews();bandText.setText(band==24?"2.4\nGHz":band==5?"5\nGHz":"6\nGHz");String[] t={"Access Points","Channel Rating","Channel Graph","Time Graph"};title.setText(t[page]);List<ScanResult> rs=results();summary.setText(bandName()+" • "+rs.size()+" réseau(x) visible(s) • toucher 2.4/5/6 GHz pour changer de bande");if(page==0)showList(rs);else if(page==1)showRating(rs);else if(page==2)showChannelGraph(rs);else showTimeGraph(rs);}
    ...
    private int signalColor(int l){return l>=-65?Color.rgb(0,190,80):l>=-75?Color.rgb(255,190,0):Color.rgb(190,70,200);}
    ...
    private void details(ScanResult r){new AlertDialog.Builder(this).setTitle(ssid(r)).setMessage("BSSID : "+r.BSSID+"\nSignal : "+r.level+" dBm\nCanal : "+channel(r.frequency)+"\nFréquence : "+r.frequency+" MHz\nLargeur : "+widthText(r.channelWidth)+"\nBande : "+bandName()+"\n\n"+r.capabilities).setPositiveButton("OK",null).show();}
