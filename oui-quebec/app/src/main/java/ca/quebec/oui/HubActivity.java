...
public class HubActivity extends Activity {
    ...
    private void build(){
        ...
        Button transition=button("🧮 Transition + commerce Québec–USA");transition.setOnClickListener(v->startActivity(new Intent(this,TransitionActivity.class)));root.addView(transition);
        Button society=button("⚜ Société civile indépendantiste — acteurs et réseaux");society.setOnClickListener(v->startActivity(new Intent(this,CivilSocietyActivity.class)));root.addView(society);
        Button app=button("📊 Données, Livre bleu et simulateur général");app.setOnClickListener(v->startActivity(new Intent(this,MainActivity.class)));root.addView(app);
        ...
        sectionTitle(root,"Mouvement, recherche et idées","La nouvelle page Société civile regroupe maintenant les principaux acteurs militants, de recherche, de diffusion, de calendrier et de financement.");
        ...
        radioStatus=tx("Prêt à écouter",14,INK,true);radioStatus.setPadding(0,dp(4),0,dp(4));root.addView(radioStatus);playButton=button("▶ Écouter QUB en direct");playButton.setOnClickListener(v->toggleRadio());root.addView(playButton);
        infoCard(root,"Méthode documentaire","Les contenus militants, partisans, éditoriaux, scientifiques, historiques et gouvernementaux sont séparés. La page Société civile distingue aussi les organismes voués directement à l’indépendance des mouvements sociaux ou nationaux plus larges.");
        TextView footer=tx("OUI Québec • V5.2 • Base stable conservée : V4.6.1",11,MUTED,true);footer.setGravity(Gravity.CENTER);footer.setPadding(0,dp(8),0,dp(8));root.addView(footer);
