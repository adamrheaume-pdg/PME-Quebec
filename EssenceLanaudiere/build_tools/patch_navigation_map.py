...
# Carte : 3 fonds, boutons de navigation, échelle et contrôles plus lisibles sur mobile.
old = "<script>const map=L.map('map',{zoomControl:true}).setView(["+centerLat+","+centerLng+"],12);L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png',{subdomains:'abcd',maxZoom:20,attribution:'© OpenStreetMap contributors © CARTO'}).addTo(map);"+js+radarSafetyJavascript(centerLat,centerLng)+foodPoiJavascript(centerLat,centerLng)+"</script></body></html>";
new = "<script>const map=L.map('map',{zoomControl:true,preferCanvas:true}).setView(["+centerLat+","+centerLng+"],12);"+
          ...
          "const Nav=L.Control.extend({options:{position:'bottomright'},onAdd:function(){const d=L.DomUtil.create('div','navBox leaflet-bar');d.innerHTML='<button id=meBtn title=Ma\u0020position>⌖</button><button id=allBtn title=Tout\u0020voir>⛽</button>';L.DomEvent.disableClickPropagation(d);return d;}});new Nav().addTo(map);"+
...
# CSS mobile: carte plus nette, boutons assez gros pour le pouce et sélecteur de fond lisible.
...
if missing: raise SystemExit('navigation/carte incomplète: '+repr(missing))

p.write_text(s,encoding='utf-8')
print('Navigation carte améliorée: fonds Clair/Rues/Satellite, recentrage, vue globale, échelle, grands contrôles')
