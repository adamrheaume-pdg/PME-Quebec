...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state){
        ...
        ip=new EditText(this); ip.setHint("Adresse IP de la télé"); ip.setSingleLine(true); ip.setText(getPreferences(MODE_PRIVATE).getString("tv_ip","")); root.addView(ip,new LinearLayout.LayoutParams(-1,dp(54)));
        Button discover=button("Découvrir"), connect=button("Connecter"); row(root,discover,connect);
        ...
        discover.setOnClickListener(v->discover()); connect.setOnClickListener(v->connectTv()); pair.setOnClickListener(v->pair());
    ...
    private void discover(){setStatus("Recherche de la Hisense...");exec.submit(()->{try{JSONArray a=new JSONArray(bridge.callAttr("discover").toString());if(a.length()==0){setStatus("Aucune télé détectée. Entre l’adresse IP manuellement.");return;}JSONObject d=a.getJSONObject(0);String host=d.getString("ip");runOnUiThread(()->ip.setText(host));setStatus("Trouvée : "+host);}catch(Exception e){setStatus("Découverte impossible : "+e.getMessage());}});}
    private void connectTv(){String host=ip.getText().toString().trim();if(host.isEmpty()){setStatus("Entre l’adresse IP.");return;}getPreferences(MODE_PRIVATE).edit().putString("tv_ip",host).apply();setStatus("Connexion...");exec.submit(()->{try{JSONObject r=new JSONObject(bridge.callAttr("connect",host,getFilesDir().getAbsolutePath()).toString());setStatus(r.optString("message"));if(r.optBoolean("ok")&&!r.optBoolean("paired"))runOnUiThread(()->pairBox.setVisibility(View.VISIBLE));}catch(Exception e){setStatus("Erreur : "+e.getMessage());}});}
