...
public class HubActivity extends Activity {
    ...
    private ImageView logo(int sizeDp){ImageView i=new ImageView(this);i.setImageResource(R.drawable.oui_logo);i.setScaleType(ImageView.ScaleType.CENTER_INSIDE);i.setContentDescription("Logo OUI Québec");i.setAdjustViewBounds(true);LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(dp(sizeDp),dp(sizeDp));p.gravity=Gravity.CENTER_HORIZONTAL;i.setLayoutParams(p);return i;}
    ...
    private void build(){
        ...
        LinearLayout head=new LinearLayout(this);head.setOrientation(LinearLayout.VERTICAL);head.setGravity(Gravity.CENTER_HORIZONTAL);head.setPadding(dp(18),dp(12),dp(18),dp(14));head.setBackgroundColor(DARK);head.addView(logo(88));TextView title=tx("OUI QUÉBEC",27,WHITE,true);title.setGravity(Gravity.CENTER);title.setPadding(0,dp(4),0,0);head.addView(title);TextView version=tx("Centre documentaire • V4.5",13,Color.rgb(210,225,250),false);version.setGravity(Gravity.CENTER);head.addView(version);outer.addView(head);
        ...
        Button app=button("Ouvrir les données, le Livre bleu et le simulateur");app.setOnClickListener(v->startActivity(new Intent(this,MainActivity.class)));root.addView(app);Button library=button("📚 Bibliothèque — indépendance du Québec");library.setOnClickListener(v->startActivity(new Intent(this,LiteratureActivity.class)));root.addView(library);Button history=button("🕰 Histoire politique — 1534 à aujourd’hui");history.setOnClickListener(v->startActivity(new Intent(this,HistoryActivity.class)));root.addView(history);
        TextView ph=tx("Partis politiques indépendantistes",22,DARK,true);ph.setPadding(0,dp(18),0,dp(6));root.addView(ph);root.addView(tx("Liens directs vers les partis et leurs positions/programmes. Cette section présente les sources partisanes comme telles; elle ne les transforme pas en données gouvernementales.",13,MUTED,false));
        ...
        card(root,"Climat Québec","Parti indépendantiste dont la plateforme 2026 articule son projet autour du bien commun et de l’indépendance du Québec.","https://climat.quebec/");
        card(root,"Archives des programmes — Assemblée nationale","Guide non partisan de la Bibliothèque de l’Assemblée nationale recensant les programmes et slogans politiques québécois, notamment ceux du PQ, de Québec solidaire et de formations indépendantistes historiques.","https://www.bibliotheque.assnat.qc.ca/guides/fr/3748-programmes-et-slogans-politiques");
        TextView h=tx("Mouvement et recherche",22,DARK,true);h.setPadding(0,dp(18),0,dp(10));root.addView(h);card(root,"Rassemblement pour un Pays Souverain (RPS)","Mouvement de la société civile, sans attaches partisanes selon sa présentation, qui promeut l’indépendance du Québec, la langue française et la connaissance de l’histoire nationale.","https://www.rps.quebec/");card(root,"L’Action nationale","Revue publiée par la Ligue d’action nationale. Carrefour souverainiste de réflexion critique sur les aspirations de la nation québécoise.","https://action-nationale.qc.ca/");card(root,"IRAI","Institut indépendant et non partisan consacré à la recherche scientifique, comparative et internationale sur l’autodétermination et l’indépendance.","https://irai.quebec/");
        TextView rh=tx("QUB radio • 99,5 FM Montréal",22,DARK,true);rh.setPadding(0,dp(12),0,dp(5));root.addView(rh);root.addView(tx("Lecteur intégré du flux numérique QUB. La programmation QUB est aussi diffusée au 99,5 FM Montréal durant certaines plages.",14,MUTED,false));radioStatus=tx("Prêt à écouter",14,INK,true);radioStatus.setPadding(0,dp(10),0,dp(5));root.addView(radioStatus);playButton=button("▶ Écouter QUB en direct");playButton.setOnClickListener(v->toggleRadio());root.addView(playButton);Button my=button("Lecteur web myTuner — QUB");my.setOnClickListener(v->open("https://mytuner-radio.com/fr/radio/qub-radio-458512/"));root.addView(my);
        TextView note=tx("Les contenus militants, partisans, éditoriaux, scientifiques, historiques et gouvernementaux sont identifiés séparément. La chronologie distingue également conflit militaire, crise politique, conflit constitutionnel et friction linguistique.",12,MUTED,false);note.setPadding(0,dp(18),0,0);root.addView(note);scroll.addView(root);outer.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));setContentView(outer);outer.requestApplyInsets();
    }
